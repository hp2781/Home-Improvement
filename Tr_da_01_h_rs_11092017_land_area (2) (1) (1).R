...
tr_da_01_h_rs3_times$land_area_hp = ifelse(is.na(tr_da_01_h_rs3_times$land_area),tr_da_01_h_rs3_times$est_land_area, tr_da_01_h_rs3_times$land_area)
...
## below is trial model run for land area..
...
m_change_area <- lm(log((contract_price_duplex_double/purchase_price_notional)) ~ 
          + DA 
        + log(ssd_index_cur_sale2/ssd_index_purchase_notional) 
        + age_notional_months_times + I(age_notional_months_times**2) 
        + I(beds_cur_sale-beds_prev_sale) 
...
m_area <- lm(log((contract_price_duplex_double/purchase_price_notional)) ~ 
          + DA 
        + log(ssd_index_cur_sale2/ssd_index_purchase_notional) 
        + age_notional_months_times + I(age_notional_months_times**2) 
        + I(beds_cur_sale)
...
m_ex_prev <- lm(log((contract_price_duplex_double/purchase_price_notional)) ~ 
          + DA 
        + log(ssd_index_cur_sale2/ssd_index_purchase_notional) 
        + age_notional_months_times + I(age_notional_months_times**2) 
        + I(beds_cur_sale)
...
m_ex_cur <- lm(log((contract_price_duplex_double/purchase_price_notional)) ~ 
          + DA 
        + log(ssd_index_cur_sale2/ssd_index_purchase_notional) 
        + age_notional_months_times + I(age_notional_months_times**2) 
      #  + I(beds_cur_sale)
...
m_ex_both <- lm(log((contract_price_duplex_double/purchase_price_notional)) ~ 
          + DA 
        + log(ssd_index_cur_sale2/ssd_index_purchase_notional) 
        + age_notional_months_times + I(age_notional_months_times**2) 
        + land_area_hp
...
stargazer(m_area, m_change_area, m, m_change, 
          # title = "" , 
          style = "aer", out = "landarea.html", float = TRUE, float.env = "sidewaystable", align = TRUE, digits = 3,
          omit = c("year","state"), 
          #  omit.labels = c("State"),
          ...
          se=list(# summary(m_ex_both,robust=T)$coef[,"Std. Error"],
                  # summary(m_ex_cur,robust=T)$coef[,"Std. Error"],
                  # summary(m_ex_prev,robust=T)$coef[,"Std. Error"],
                   summary(m_area,robust=T)$coef[,"Std. Error"],
                   summary(m_change_area,robust=T)$coef[,"Std. Error"],
...
## above is trial model run for land area..
...
## below is for propensity scores matching using logistic regression for correcting for self selection bias..

tr_da_01_h_rs3_times_select = tr_da_01_h_rs3_times[tr_da_01_h_rs3_times$state!="ACT"& tr_da_01_h_rs3_times$state != "NT" 
                                                   ,c("building_type_compact2","state","suburb","postcode","SSDcode", 
                                                      "property_id", "property_type", "DA","contract_date2", "contract_price_duplex_double", 
                                                      "purchase_price_notional" ,"land_area_hp", "prev_sale_date2", "prev_sale_price",
                                                      "beds_cur_sale","baths_curr_sale","cars_curr_sale","beds_prev_sale","baths_prev_sale",
                                                      "cars_prev_sale","ssd_index_cur_sale2","ssd_index_prev_sale2", 
                                                      "months_bet_sales", "age_notional_months_times")]
...
tr_da_01_h_rs3_times_select_nomiss <- tr_da_01_h_rs3_times_select %>%  # MatchIt does not allow missing values
  #  select(c5r2mtsc_std, catholic, one_of(ecls_cov)) %>%
  na.omit()

mps = glm(DA ~ log(prev_sale_price) + log(land_area_hp)+ 
            beds_cur_sale + baths_curr_sale + cars_curr_sale + 
            beds_prev_sale + baths_prev_sale + cars_prev_sale + 
            log(ssd_index_cur_sale2) + log(ssd_index_prev_sale2) + 
            months_bet_sales + I(months_bet_sales**2) + as.factor(state), 
          family = binomial(), data = tr_da_01_h_rs3_times_select)

prs_df <- data.frame(pr_score = predict(mps, type = "response"),
                     DA = mps$model$DA)
head(prs_df)
...
labs <- paste("Actual group:", c("Improved", "Unimproved"))
prs_df %>%
  mutate(DA = ifelse(DA == 1, labs[1], labs[2])) %>%
  ggplot(aes(x = pr_score)) +
  geom_histogram(color = "white", bins = 100) +
  facet_wrap(~DA) +
  xlab("Probability of Improved Home") +
  theme_bw()
...
mod_match <- matchit(DA ~ log(prev_sale_price) + log(land_area_hp)+ 
                       beds_cur_sale + baths_curr_sale + cars_curr_sale + 
                       beds_prev_sale + baths_prev_sale + cars_prev_sale + 
                       log(ssd_index_cur_sale2) + log(ssd_index_prev_sale2) + 
                       months_bet_sales + I(months_bet_sales**2) + as.factor(state), 
                     method = "nearest", data = tr_da_01_h_rs3_times_select_nomiss)
summary(mod_match)

tr_da_01_h_rs3_times_select_nomiss_match <- match.data(mod_match)
dim(tr_da_01_h_rs3_times_select_nomiss_match)

tr_da_01_h_rs3_times_match = merge(tr_da_01_h_rs3_times, tr_da_01_h_rs3_times_select_nomiss_match[,c("property_id","state", "SSDcode", "prev_sale_date2", "contract_date2", "distance")], by = c("property_id","state", "SSDcode", "prev_sale_date2", "contract_date2"), all.x = TRUE)
dim(tr_da_01_h_rs3_times_match[tr_da_01_h_rs3_times_match$DA==0 & !is.na(tr_da_01_h_rs3_times_match$distance),])
...
m_da <- lm(log((contract_price_duplex_double/purchase_price_notional)) ~ 
           ...
           + I(cars_curr_sale - cars_prev_sale)
           + as.factor(year(purchase_date_notional)) + as.factor(state) 
           ,(selfselect2 == 1 | selfselect2 == s) & !is.na(distance) &
             prev_sale_price >= 40000 & prev_sale_price<=5500000 &
             contract_price >= 40000 & contract_price <=5500000 & 
             purchase_date_notional >= as.Date("2004-03-01") & 
             vacantb==0 & vacant_s==0 & vacant_bs ==0 
           , data = tr_da_01_h_rs3_times_match )
...
### 1. below is creating the table now... run with selfselect = 0  this is for correcting self selection bias..

file_hp = capture.output(cat('self_select_correct',s,'.html',sep = ''))
...
## above is for propensity scores matching using logistic regression for correcting for self selection bias..
